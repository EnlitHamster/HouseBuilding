...
class RQueueSpec extends AnyFlatSpec {
  ...
  it must s"throw a NPE when adding a null element" in assertThrows[NullPointerException]{val test: String = null; VoidQueue[String] + test}
  it must s"throw a NPE when adding a null arrays" in assertThrows[NullPointerException]{val test: Array[String] = null; VoidQueue[String] ++ test}
